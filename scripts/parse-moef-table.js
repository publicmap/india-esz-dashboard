...
// into structured records, one per Draft or Final notification, written to
// data/moef/esz-notifications.{json,csv}. Sibling to parse-wikipedia-tables.js,
// which does the analogous job for the Wikipedia protected-area list tables.
//
...
const OUTPUT_JSON_PATH = 'data/moef/esz-notifications.json';
const OUTPUT_CSV_PATH = 'data/moef/esz-notifications.csv';
...
const GENERIC_PA_WORDS_RE = /\b(national|park|wildlife|santuary|sanctuary|reserve|forest|tiger|bird|wls|np|esz)\b/gi;

function significantWords(name) {
  return new Set(
    name.toLowerCase().replace(GENERIC_PA_WORDS_RE, ' ').split(/\W+/).filter((w) => w.length >= 3),
  );
}

// An anchor's title attribute is normally more reliable than its own visible
// text (which is often boilerplate like "Draft ESZ Notification on X"), so
// extractProtectedAreaName prefers it below. But the source table is hand-
// maintained, and a row's title occasionally gets copy-pasted from a
// different, unrelated PA's row and never updated (e.g. a Campbell Bay
// National Park link titled "Mount Harriet National Park") while its href and
// visible text are correct. Requiring the title to share at least one
// significant (non-generic, 3+ letter) word with that same anchor's own text
// catches that case without rejecting the common minor-typo case (e.g.
// title "Bandh Baretha Wildlife" vs text "Bondh Baretha Wildlife Sanctuary"),
// where falling through to the text-based name below is harmless.
function titleAgreesWithOwnText(title, ownText) {
  const ownWords = significantWords(ownText);
  if (ownWords.size === 0) return true;
  const titleWords = significantWords(title);
  for (const w of titleWords) if (ownWords.has(w)) return true;
  return false;
}
...
async function main() {
  ...
  await mkdir('data/moef', { recursive: true });
  await writeFile(OUTPUT_JSON_PATH, JSON.stringify(records, null, 2), 'utf8');
  ...
  await writeFile(OUTPUT_CSV_PATH, csv, 'utf8');
